...
function MotionHighlight<T extends string>({ ref, ...props }: MotionHighlightProps<T>) {
  ...
  const boundsOffset = React.useMemo(() => {
    return (
      (props as ParentModeMotionHighlightProps)?.boundsOffset ?? {
        top: 0,
        left: 0,
        width: 0,
        height: 0,
  ...
  const containerClassName = React.useMemo(() => {
    return (props as ParentModeMotionHighlightProps)?.containerClassName
  }, [props])
  ...
  const forceUpdateBoundsValue = React.useMemo(() => {
    return (props as ParentModeMotionHighlightProps)?.forceUpdateBounds
  }, [props])
  ...
  return (
    <MotionHighlightContext.Provider
      value={{
        ...
        forceUpdateBounds: forceUpdateBoundsValue,
      }}
    >
      {enabled
        ? controlledItems
